...
import {googlePlaceUrl,naverAppUrl,naverWebUrl,resolvedMapPlaceName} from "../lib/map-links";
import {
  CalendarDays,
  Car,
  Camera,
...
export default function Home() {
  ...
  function openRoute(provider:MapProvider,day:string){const points=mapDays.find(item=>item.date===day)?.points||[];setSelectedMapDate(day);setSelectedMapPoint("");if(points.length<2){setMapPicker(null);setNotice("이 날짜에는 경로를 만들 좌표가 부족합니다");return}if(provider==="google"){setMapPicker({provider,day,routes:points.map(point=>[point])});return}const routes=splitContinuous(points,2);if(routes.length>1){setMapPicker({provider,day,routes});return}const picker={provider,day,routes};setMapPicker(null);openNaverRoute(routes[0],picker)}
  ...
  function openChosenRoute(index:number){if(!mapPicker?.day||!mapPicker.routes)return;const points=mapPicker.routes[index],picker={provider:mapPicker.provider,day:mapPicker.day,routes:mapPicker.routes};setMapPicker(null);if(mapPicker.provider==="google")openExternal(googlePlaceUrl(points[0]),picker);else openNaverRoute(points,picker)}
  return (
    <main>
      <header>
        <div className="brand">
      ...
      {mapPicker&&<MapRouteModal picker={mapPicker} days={mapDays} close={()=>setMapPicker(null)} chooseDay={day=>openRoute(mapPicker.provider,day)} chooseRoute={openChosenRoute}/>}
...
function MapRouteModal({picker,days,close,chooseDay,chooseRoute}:{picker:MapPicker;days:MapDay[];close:()=>void;chooseDay:(date:string)=>void;chooseRoute:(index:number)=>void}){const title=picker.provider==="google"?"구글 지도 방문 위치 보기":"네이버 지도 자동차 길찾기";return <Modal title={title} close={close}><div className="map-route-picker">{picker.fallbackUrl?<><p>팝업 또는 지도 앱을 열지 못했습니다. 아래 링크를 눌러 웹 지도를 열어주세요.</p><a href={picker.fallbackUrl} target="_blank" rel="noreferrer">웹 지도 링크 열기</a></>:!picker.day?<><p>확인할 날짜를 선택하세요.</p>{picker.provider==="google"&&<p className="map-provider-note">구글 지도에서는 국내 자동차 경로가 표시되지 않을 수 있습니다.</p>}{days.map((day,index)=><button key={day.date} type="button" onClick={()=>chooseDay(day.date)}><b>{tripDayLabel(index+1)}</b><span>{Number(day.date.slice(5,7))}월 {Number(day.date.slice(8,10))}일 · 방문 장소 {day.points.length}곳</span></button>)}</>:<><p>{picker.provider==="google"?"방문 위치를 시간순으로 확인하세요. 구글 지도에서는 국내 자동차 경로가 표시되지 않을 수 있습니다.":"네이버 지도에서 열 자동차 길찾기 구간을 선택하세요."}</p>{picker.routes?.map((route,index)=>picker.provider==="google"?<a key={index} href={googlePlaceUrl(route[0])} target="_blank" rel="noreferrer"><b>방문 위치 {index+1}</b><span>{route[0].name}</span></a>:<div className="naver-route-choice" key={index}><button type="button" onClick={()=>chooseRoute(index)}><b>구간 {index+1}</b><span>{route[0].name} → {route.at(-1)!.name}</span></button><a href={naverWebUrl(route)} target="_blank" rel="noreferrer">웹 지도</a></div>)}</>}</div></Modal>}
...
function TimelineCards({records,selectedMoves,setSelectedMoves,openMerge,onPreviewPlaces,onLookup,onEdit,onDelete,onSplit,onNotVisit}: {records:TimelineRecord[]}&TimelineActions){if(!records.length)return null;const ordered=[...records].sort((a,b)=>a.startTime.localeCompare(b.startTime)),firstDate=ordered[0].startTime.slice(0,10);return <div className="imported-records"><div className="merge-bar"><span>이동 기록의 선택칸을 눌러 합칠 기록만 고르세요.</span><button onClick={onPreviewPlaces}>저장 장소 적용</button><button onClick={openMerge} disabled={selectedMoves.length<2}>선택한 이동 {selectedMoves.length}개 합치기</button></div>{ordered.map((r,index)=>{const date=r.startTime.slice(0,10),previousDate=index?ordered[index-1].startTime.slice(0,10):"",newDay=index>0&&date!==previousDate;return <Fragment key={`timeline-${r.id}`}>{newDay&&<div className="timeline-date-boundary"><TimelineDayMarker date={date} firstDate={firstDate}/><div className="midnight-time">00:00</div><span aria-hidden="true"/></div>}<article className={`timeline-record ${r.recordType}`}><div className="day-marker-slot">{index===0&&<TimelineDayMarker date={date} firstDate={firstDate}/>}</div><div className="record-time"><b>{r.startTime.slice(11,16)}</b><span>~ {r.endTime?.slice(11,16)||"--:--"}</span></div><div className="record-body">{r.recordType==="activity"&&<label className="move-check"><input type="checkbox" checked={selectedMoves.includes(r.id)} onChange={e=>setSelectedMoves(old=>e.target.checked?[...old,r.id]:old.filter(id=>id!==r.id))}/><Car/>합칠 이동 선택</label>}<small>{r.recordType==="activity"?"이동 기록":"방문 기록"}</small><h3 className={r.title==="장소명 입력 필요"?"missing-place":""}>{r.title}</h3>{r.recordType==="activity"?<div className="movement-detail"><p><b>이동수단</b> {r.transportType||r.originalTransportType||"알 수 없음"}</p><p><b>출발</b> {r.startPlace||"출발지 입력 필요"}</p><p><b>도착</b> {r.endPlace||"도착지 입력 필요"}</p><p><b>이동시간</b> {durationText(r.startTime,r.endTime)} · <b>거리</b> {r.distanceMeters!=null?`${(r.distanceMeters/1000).toFixed(2)}km`:"알 수 없음"}</p><p><b>출발 좌표</b> {coordinate(r.startLat,r.startLng)}</p><p><b>도착 좌표</b> {coordinate(r.endLat,r.endLng)}</p></div>:<><p>{r.note}</p><p><b>방문시간</b> {r.startTime.replace("T"," ").slice(0,16)} ~ {r.endTime?.replace("T"," ").slice(0,16)||"알 수 없음"}</p><p><b>좌표</b> {coordinate(r.startLat,r.startLng)}</p></>}<div className="record-actions">{r.recordType==="visit"&&<><button onClick={()=>onLookup(r)}><Search/>장소명 찾기</button><button onClick={()=>r.endTime?onSplit(r):alert("끝 시간이 있는 방문 기록만 나눌 수 있습니다.")}><Scissors/>나누기</button><button onClick={()=>onNotVisit(r)}>방문 아님</button></>}<button onClick={()=>onEdit(r)}><Pencil/>수정</button><button className="delete" onClick={()=>onDelete(r)}><Trash2/>삭제</button></div></div></article></Fragment>})}</div>}
