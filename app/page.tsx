...
function correctedVisitCoordinates(record:TimelineRecord){const lat=Number(record.startLat),lng=Number(record.startLng),hotelRocio=record.title.trim()==="호텔 로씨오"&&Math.abs(lat-36.969646)<0.000001&&Math.abs(lng-126.781488)<0.000001;return hotelRocio?{lat:"36.969541",lng:"126.7815682"}:{lat:record.startLat,lng:record.startLng}}
...
function buildMapDays(trip:Trip,records:TimelineRecord[]):MapDay[]{const dates=dateRange(trip.startDate,trip.endDate),visits=[...records].filter(record=>record.recordType==="visit").sort((a,b)=>a.startTime.localeCompare(b.startTime));return dates.map(date=>{const points:MapPoint[]=[];for(const record of visits){const startDate=record.startTime.slice(0,10),endDate=record.endTime?.slice(0,10),arrival=clock(record.startTime),departure=clock(record.endTime),name=resolvedMapPlaceName(record),coordinates=correctedVisitCoordinates(record);if(startDate===date){const crossesDate=!!endDate&&endDate>date,markerText=arrival?crossesDate?`${arrival} 도착`:arrival:name,detailText=arrival?`도착 ${arrival}${departure?` · 출발 ${crossesDate?"다음 날 ":""}${departure}`:""}`:"";const point=validMapPoint(coordinates.lat,coordinates.lng,name,record.startTime,{arrivalTime:arrival,departureTime:departure,markerText,detailText,important:crossesDate});if(point)points.push(point)}else if(startDate<date&&!!endDate&&endDate===date){const markerText=departure?`${departure} 출발`:name,detailText=`${arrival?`도착 전날 ${arrival}`:""}${arrival&&departure?" · ":""}${departure?`출발 ${departure}`:""}`;const point=validMapPoint(coordinates.lat,coordinates.lng,name,`${date}T00:00:00`,{arrivalTime:arrival,departureTime:departure,markerText,detailText,important:true});if(point)points.push(point)}}const deduplicated=points.sort((a,b)=>a.time.localeCompare(b.time)).filter((point,index,list)=>index===0||!sameMapPoint(point,list[index-1]));return{date,points:deduplicated}})}
